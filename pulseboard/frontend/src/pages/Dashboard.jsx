...
const CRISIS_DATA = {
  revenue: { yesterday: 8450, day_before: 9800, weekly_avg: 9200, refunds_today: 1240, failed_payments: 89 },
...
function StatCard({ label, value, sub, color, trend = 0 }) {
  return (
    <TiltCard className="glass-panel rounded-2xl p-6 relative overflow-hidden group" intensity={4}>
      ...
      <div className="text-xs font-semibold tracking-wider text-text-muted uppercase mb-2">{label}</div>
...
export default function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    return { total: briefs.length, avg: avg.toFixed(1), health: healthMeta(briefs[0].overall_health), chartData };
  }, [briefs]);
  ...
  if (error) return (
    <div className="max-w-6xl mx-auto px-6 py-10 text-status-critical bg-status-critical/10 p-4 rounded-xl border border-status-critical/20">{error}</div>
  );
  ...
  if (!briefs?.length) return (
    <div className="max-w-4xl mx-auto px-6 py-20 text-center">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-panel rounded-3xl p-12 flex flex-col items-center">
        ...
        <div className="flex items-center gap-4">
          <Link to="/new" className="btn-primary px-6 py-3 rounded-xl font-medium tracking-wide">Upload CSV</Link>
          <button onClick={handleDemo} disabled={demoLoading} className="btn-secondary px-6 py-3 rounded-xl font-medium">
    ...
    <div className="max-w-6xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="flex flex-col md:flex-row gap-8 justify-between items-start md:items-center relative z-10">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-sm font-medium text-text-muted">Last updated {formatTimestamp(latest.created_at)}</span>
            </div>
            <h1 className="text-3xl md:text-4xl font-bold text-text-primary mb-4 leading-tight">{stats.health.phrase}.</h1>
          ...
          <div className="shrink-0 flex flex-col gap-3">
            ...
            <button onClick={handleDemo} disabled={demoLoading} className="btn-secondary px-8 py-3 rounded-xl font-medium text-center text-sm">
              {demoLoading ? "Running..." : "⚡ Run New Analysis"}
            </button>
          </div>
        </div>
      </motion.div>
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={stats.chartData}>
                ...
                <Tooltip contentStyle={{ backgroundColor: "var(--bg-card)", borderColor: "var(--bg-border)", borderRadius: "12px", color: "var(--text-primary)" }} itemStyle={{ color: "var(--brand-red)" }} />
                ...
                <Link key={brief.id} to={`/brief/${brief.id}`} className="block group">
                  <div className="p-4 rounded-xl border border-bg-border bg-bg-primary/30 hover:bg-bg-card transition-colors">
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-medium text-text-primary truncate text-sm">{brief.data_source_name}</span>
                      <span className="text-xs px-2 py-0.5 rounded uppercase font-bold ml-2 shrink-0" style={{ color: meta.color, background: meta.tint }}>{meta.label}</span>
                    </div>
                    <div className="text-xs text-text-subtle">{formatTimestamp(brief.created_at)}</div>
                  </div>
                </Link>
              );
            })}
      ...
      <div className="text-center py-4">
        <p className="text-xs text-text-muted">
          Powered by <span className="text-brand-red font-medium">AMD Developer Cloud</span> · Fireworks AI · LLaMA 3.1 70B · Team Cogent
        </p>
      </div>
    </div>
  );
