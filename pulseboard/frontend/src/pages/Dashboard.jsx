...
const CRISIS_DATA = {
  ...
  users: {
    active_today: 1243,
    active_yesterday: 1891,
    churned_this_week: 47,
    new_signups_today: 12,
  },
  errors: {
    error_rate_percent: 8.3,
    normal_error_rate: 0.8,
    checkout_failures: 143,
    api_timeouts: 67,
    peak_error_time: "11:30 PM",
  },
  sentiment: {
    support_tickets_today: 28,
    normal_tickets_per_day: 6,
    nps_score: 31,
    nps_last_month: 58,
    top_complaint: "checkout not working on mobile",
  },
};
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 text-sm text-text-subtle">
        {trend !== 0 && (
          <span
            className={trend > 0 ? "text-status-good" : "text-status-critical"}
          >
...
export default function Dashboard() {
  ...
  const handleDemo = async () => {
    ...
    try {
      ...
      setTimeout(async () => {
        await refresh();
        setDemoPhase("idle");
        // Force full reload so dashboard shows new brief
      ...
      <div className="max-w-6xl mx-auto px-6 py-10 animate-pulse">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-32 bg-bg-card/50 rounded-2xl border border-bg-border"
            />
          ))}
        </div>
      </div>
  ...
  if (error) {
    return (
      <div className="max-w-6xl mx-auto px-6 py-10 text-status-critical bg-status-critical/10 p-4 rounded-xl border border-status-critical/20">
        {error}
      </div>
    );
  }

  // ── Agent Activity (demo running) ────────────────────────
  if (demoPhase !== "idle") {
    return (
      <div className="mx-auto max-w-4xl px-6 pt-16 pb-16">
        ...
        >
          ...
          <p className="text-text-muted text-lg">
            Five specialist AI agents are processing the crisis scenario
            dataset.
          </p>
        </motion.div>
        <AgentActivity
          active={demoPhase === "analyzing" || demoPhase === "done"}
          result={demoResult}
        />
      </div>
    );
      ...
      <div className="max-w-4xl mx-auto px-6 py-20 text-center">
        ...
        >
          ...
          <p className="text-text-muted text-lg max-w-lg mb-10">
            Upload your first CSV to generate AI-powered executive intelligence.
            PulseBoard's multi-agent system will analyze it instantly.
          </p>
          <div className="flex items-center gap-4">
            <Link
              to="/new"
              className="btn-primary px-6 py-3 rounded-xl font-medium tracking-wide"
            >
              Upload CSV
            </Link>
            <button
    ...
    <div className="max-w-6xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="flex flex-col md:flex-row gap-8 justify-between items-start md:items-center relative z-10">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-4">
              <div
                className="px-3 py-1 text-[10px] font-bold tracking-[0.2em] rounded-full uppercase"
                style={{
                  background: stats.health.tint,
                  color: stats.health.color,
                  border: `1px solid ${stats.health.border}`,
                }}
              >
                {stats.health.label}
              </div>
            ...
            <p className="text-text-subtle text-lg leading-relaxed max-w-2xl">
              {primaryStory(latest.final_brief) ||
                "The multi-agent system has completed its latest analysis pass."}
            </p>
          </div>
          ...
          <div className="shrink-0 flex flex-col gap-3">
            <Link
              to={`/brief/${latest.id}`}
              className="btn-primary px-8 py-3.5 rounded-xl font-semibold text-center hover:scale-105 transition-transform"
            >
            ...
            <button
              onClick={handleDemo}
              disabled={demoLoading}
              className="btn-secondary px-8 py-3 rounded-xl font-medium text-center text-sm"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={`${stats.avg}s`}
          sub="5 agents · AMD parallel compute"
          color="#F59E0B"
          trend={-12}
        />
        <StatCard
          label="Data Sources"
          value={new Set(briefs.map((b) => b.data_source_name)).size}
          sub="Unique connections"
          color="#10B981"
        />
        <StatCard
          label="Anomalies"
          value={latest.anomalies?.length || 0}
          sub="In latest brief"
          color="#EF4444"
          trend={5}
        />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-panel rounded-2xl p-6">
          <h3 className="text-sm font-semibold tracking-wider text-text-muted uppercase mb-6">
                ...
                >
                  <div className="p-4 rounded-xl border border-bg-border bg-bg-primary/30 hover:bg-bg-card transition-colors">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span
                        className="text-xs px-2 py-0.5 rounded uppercase font-bold ml-2 shrink-0"
      ...
      <div className="text-center py-4">
        <p className="text-xs text-text-muted">
          ...
          {" · "}Fireworks AI · LLaMA 3.1 70B · Team Cogent
        </p>
      </div>
    </div>
  );
